...
t2 = 0:T_2ms:3*T-T_2ms; % perioada semnalului esantionat cu Ts= 2 ms(parcurg 3 perioade)
...
t20= 0:T_20ms:3*T-T_20ms; % perioada semnalului esantionat cu Ts= 20ms
...
t200 = 0:T_200ms:3*T-T_200ms; % perioada semnalului esantionat cu Ts = 200ms
...
x2 = sawtooth(t2(t2<=T-T_2ms)*(2*pi/T),3/5); % panta+ = 1V/s ; panta- = -1.5 V/s
x2 = x2 * 1.5 - 0.5;
x2 =[x2,x2,x2];
...
x20 = sawtooth(t20(t20<=T-T_20ms)*(2*pi/T),3/5); % panta+ = 1V/s ; panta- = -1.5 V/s
x20 = x20 * 1.5 - 0.5;
x20 =[x20,x20,x20];
...
x2=ones(1,0.25/T_2ms); % crearea unui vector cu nr. de esantioane necesar
...
% punerea unor conditii pentru a imparti numerele in categorii.Impartim
